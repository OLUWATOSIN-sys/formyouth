...
export default function AdminPage() {
  ...
  const fetchRegistrations = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/registrations?t=${Date.now()}`, {
        cache: 'no-store'
      });
  ...
  const handleConfirmPayment = async (id: string, isManual: boolean = false) => {
    const message = isManual 
      ? "Manually mark this registration as PAID? This will confirm payment without proof upload."
    ...
    try {
      ...
      if (response.ok) {
        // Force refresh registrations
        await fetchRegistrations();
        alert(isManual ? "Payment marked as paid successfully!" : "Payment confirmed successfully!");
      } else {
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-1 flex-wrap">
                          ...
                          {/* Manual Mark as Paid button for not_paid status */}
                          {reg.paymentStatus === "not_paid" && (
                            <button
                              onClick={() => handleConfirmPayment(reg._id, true)}
                              disabled={actionLoading === `confirm-${reg._id}`}
                              className="px-2 py-1 bg-yellow-500/20 text-yellow-400 rounded hover:bg-yellow-500/30 text-xs disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1 whitespace-nowrap"
