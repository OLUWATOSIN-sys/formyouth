...
export default function VotingPart2Page() {
  ...
  useEffect(() => {
    ...
    // Check if user has already voted on Part 2 (permanent restriction)
    const hasVotedPart2 = localStorage.getItem("hasVotedPart2");
    if (hasVotedPart2 === "true") {
      setAlreadyVoted(true);
    }
    
    // Load saved votes from localStorage
    const savedVotes = localStorage.getItem("votingPart2");
    if (savedVotes) {
  ...
  const handleVoteChange = (categoryId: string, nominee: string) => {
    ...
    localStorage.setItem("votingPart2", JSON.stringify(newVotes));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        localStorage.setItem("hasVotedPart2", "true");
        localStorage.removeItem("votingPart2");
        setSubmitted(true);
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="text-center pt-6">
              ...
              >
                {loading ? "Submitting..." : "Submit All Votes"}
              </button>
            </div>
          </form>
        </div>
