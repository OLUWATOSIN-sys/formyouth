...
export default function VoteCommittedPage() {
  ...
  useEffect(() => {
    ...
    const hasVoted = localStorage.getItem("hasVoted_most_committed");
    if (hasVoted === "true") setAlreadyVoted(true);
    const savedVote = localStorage.getItem("vote_most_committed");
    if (savedVote) setSelectedNominee(savedVote);
  }, []);

  const handleVoteChange = (nominee: string) => {
    setSelectedNominee(nominee);
    localStorage.setItem("vote_most_committed", nominee);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        localStorage.setItem("hasVoted_most_committed", "true");
        localStorage.removeItem("vote_most_committed");
        setSubmitted(true);
